...
"""Custom permission rules for Buzz so users can see records linked to them
...
def event_booking_has_permission(doc, ptype, user):
	"""Allow read on Event Booking when the user is the linked user OR the owner."""
	if user == "Administrator":
		return True
...
def event_ticket_has_permission(doc, ptype, user):
	"""Allow read on Event Ticket when the user is the attendee OR the owner."""
	if user == "Administrator":
		return True
