...
const addAttendee = () => {
	...
	if (ticketType && ticketTypesMap.value[ticketType]) {
		...
		const remainingTickets = ticketInfo.remaining_tickets
		if (remainingTickets !== undefined && remainingTickets >= 0) {
			const currentCount = attendees.value.filter(
				(attendee) => String(attendee.ticket_type) === String(ticketType),
			).length
			if (currentCount >= remainingTickets) {
				toast.warning(
					__("Only {0} tickets available for {1}", [remainingTickets, ticketInfo.title]),
				)
			}
		}
	}
